...
def scrape_rates():
    ...
    try:
        ...
        rates = []
        # Find the table with exchange rates (the second table on the page)
        tables = soup.find_all('table')
        if len(tables) >= 2:
            table = tables[1]  # Get the second table which contains the bank rates
            ...
            for row in rows:
                ...
                if len(cols) >= 3:
                    ...
                    # Only add if both buy and sell rates are present
